...
class Auth extends ShieldAuth
{
     ...
     * 
     * Se o usuário entrou via Magic Link, redireciona para a troca de senha.
     */
    public function loginRedirect(): string
    {
        $session = session();

        // Magic Link seta uma flag de sessão — detectamos para redirecionar
        if ($session->get('magicLinkUsed') || $session->getTempdata('magicLinkToken')) {
            $session->remove('magicLinkUsed');
            return site_url('perfil/trocar_senha');
        }
